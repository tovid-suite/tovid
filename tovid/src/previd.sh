...
OPTIONS may be any of the following:
    ...
    -x NUM
    -y NUM
        Sets the desired width and height, respectively, of the output video.
        These must be multiples of 16.

    -sstep NUM
        Skip NUM seconds between frame captures. Larger numbers produce
        faster-running previews.
    -fps NUM
        Show each captured frame NUM times. This effectively slows the preview
        so it resembles a fast slideshow.
EOF`
...
TMP_DIR=`mktemp -d previd-temp.XXXXXX`
CWD=`pwd`
